...
type ReferenceIndex = ReadonlyMap<number, Variable | null>;

function buildReferenceIndex(scopes: Scopes): ReferenceIndex {
  const index = new Map<number, Variable | null>();
  for (const scope of scopes) {
    for (const reference of scope.references) {
      index.set(reference.identifier.start, reference.resolved);
    }
  }
  return index;
}
...
function resolvedVariableForIdentifier(
  ...
  return references.get(identifier.start) ?? null;
}
...
function knownValueEvidence(
  ...
  const variable = resolvedVariableForIdentifier(references, unwrapped);
...
export const noWidenThenAssertRule = defineRule({
  ...
  createOnce(context) {
    let references: ReferenceIndex = new Map();
    ...
    const checkAssertion = (node: ESTree.TSAsExpression | ESTree.TSTypeAssertion) => {
      ...
      const variable = resolvedVariableForIdentifier(references, expression);
      if (variable === null) return;
      const widened = widenedBinding(variable, references, environment);
      if (
        widened === null ||
    ...
    return {
      Program(node) {
        references = buildReferenceIndex(context.sourceCode.scopeManager.scopes);
